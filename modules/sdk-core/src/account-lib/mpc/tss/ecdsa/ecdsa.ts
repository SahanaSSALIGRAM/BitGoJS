...
export default class Ecdsa {
   ...
   * @deprecated - use keyShareStep1() followed by keyShareStep2() instead
   ...
   * @param {number} numShares  Number of shares
   * @param {Buffer} seed optional 64 byte seed to use for key generation
   * @returns {UYShare} Returns share of private u value and commitment to public y value.
   */
  keyShareStep1(index: number, threshold: number, numShares: number, seed?: Buffer): UYShare {
    if (!(index > 0 && index <= numShares && threshold <= numShares && threshold === 2)) {
      throw 'Invalid KeyShare Config';
    }
    ...
    return {
      ...
      c: numShares,
      uu: u.toString(),
      comDecomY: HashCommitment.createCommitment(bigIntToBufferBE(y, 33)),
      chaincode: chaincode.toString('hex'),
    };
  }
  ...
  /**
   * Generate shares for participant at index and split keys `(threshold,numShares)` ways.
   * @param {UYShare} uyShare share generated by keyShareStep1()
   ...
   * @returns {Promise<KeyShare>} Returns the private p-share
   * and n-shares to be distributed to participants at their corresponding index including decommitment to public y value.
   */
  async keyShareStep2(uyShare: UYShare, sync = false): Promise<KeyShare> {
    // Generate additively homomorphic encryption key.
    let paillierKeyPair: paillierBigint.KeyPair;
    if (!sync) {
      paillierKeyPair = await paillierBigint.generateRandomKeys(minModulusBitLength, true);
    ...
    // Compute secret shares of the private key
    const { shares: uShares, v } = Ecdsa.shamir.split(BigInt(uyShare.uu), uyShare.t, uyShare.c);
    const currentParticipant: PShare = {
      i: uyShare.i,
      t: uyShare.t,
      c: uyShare.c,
      ...
      m: bigIntToBufferBE(privateKey.mu, 192).toString('hex'),
      n: bigIntToBufferBE(publicKey.n, 384).toString('hex'),
      y: uyShare.comDecomY.decommitment.secret.toString('hex'),
      u: bigIntToBufferBE(uShares[uyShare.i], 32).toString('hex'),
      uu: uyShare.uu,
      chaincode: uyShare.chaincode,
    };
    const keyShare: KeyShare = {
      pShare: currentParticipant,
      nShares: {},
    ...
    for (const share in uShares) {
      const participantIndex = parseInt(share, 10);
      if (participantIndex !== uyShare.i) {
        keyShare.nShares[participantIndex] = {
          i: participantIndex,
          j: currentParticipant['i'],
          n: publicKey.n.toString(16),
          y: uyShare.comDecomY.decommitment.secret.toString('hex'),
          decomY: uyShare.comDecomY.decommitment,
          v: bigIntToBufferBE(v[0], 33).toString('hex'),
          u: bigIntToBufferBE(uShares[participantIndex], 32).toString('hex'),
          chaincode: uyShare.chaincode,
        } as NShare;
  ...
  keyCombine(pShare: PShare, nShares: NShare[]): KeyCombined {
    nShares.map((share) => {
      if (share.decomY && share.comY) {
        if (!HashCommitment.verifyCommitment(share.comY, share.decomY)) {
          throw new Error(`Could not verify commitment to y from participant ${share.j}`);
        }
      }
    });
